...
jpeg("FPG-mean-residual-life-plot-60-right.jpeg", quality = 100, units = "in", width = 7, height = 6, res = 500)
# 单变量过程找到 FPG 的阈值
mrlplot(FPG, xlab = "Threshold u (FPG)")
...
# 添加文本标签在竖线的右侧
text(x = 6.44, y = text_y_position, labels = "u=6.44", pos = 4, col = "red", xpd = TRUE, adj = 0)

# 保存图像到jpeg文件
jpeg("BMI-mean-residual-life-plot-60-right.jpeg", quality = 100, units = "in", width = 7, height = 6, res = 500)
# 单变量过程找到 mean_ref_Cumulative_BMI_years 的阈值
mrlplot(mean_ref_Cumulative_BMI_years, xlab = "Threshold u (Mean Excess BMI-years)")
...
jpeg("BMI-QQ-plot-60-85th.jpeg", units = "in", width = 7, height = 6, res = 500)
gpd.diag(gpdfit_cu_bmi) 
dev.off()  # 关闭设备
# 打印出尺度和形状参数及其标准误
