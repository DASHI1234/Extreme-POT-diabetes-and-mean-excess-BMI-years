...
ggplot(data1, aes(x = Age_Group, y = x)) +
  ...
  # 设置图形标题和轴标签
  labs(title = "", x = "Group", y = expression(chi ~ "(95% CI)")) +
  # 设置主题样式
  theme_minimal() +
  theme(
  ...
  # 添加红色虚线标注误差范围
  annotate("segment", x = 1, xend = 1, y = data1$CI_Lower[1], yend = data1$CI_Upper[1], 
           linetype = "dashed", color = "red") +
  annotate("segment", x = 2, xend = 2, y = data1$CI_Lower[2], yend = data1$CI_Upper[2], 
           linetype = "dashed", color = "red") +
  # 在每个点下方标注误差范围的文本
  annotate("text", x = 1, y = data1$CI_Lower[1] - 0.02, 
           label = sprintf("CI: [%.3f, %.3f]", data1$CI_Lower[1], data1$CI_Upper[1]), 
           size = 3, hjust = 0.5, colour = "black") +
  annotate("text", x = 2, y = data1$CI_Lower[2] - 0.02, 
           label = sprintf("CI: [%.3f, %.3f]", data1$CI_Lower[2], data1$CI_Upper[2]), 
           size = 3, hjust = 0.5, colour = "black")
...
jpeg("Annualized_reference-x_95CI_age_group-add broder.jpeg", units = "in", width = 7, height = 6, res = 500)
# 绘制图形
ggplot(data1, aes(x = Age_Group, y = x)) +
  # 添加点
  geom_point(size = 3, colour = 'skyblue') +
